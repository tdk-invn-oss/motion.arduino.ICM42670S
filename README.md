...
# ICM42670S Arduino library
This arduino library is for the [TDK/Invensense ICM42670S High Performance 6-Axis MotionTracking <sup>(TM)</sup> IMU with Motion Processing Software Library](https://invensense.tdk.com/products/motion-tracking/6-axis/icm-42670-s).
The ICM42670S is a high performance 6-axis MEMS MotionTracking device that combines a 3-axis gyroscope and a 3-axis accelerometer. It has a configurable host interface that supports I3C<sup>SM</sup>, I2C, and SPI serial communication, features up to 2.25 Kbytes FIFO and 2 programmable interrupts with ultra-low-power wake-on-motion support to minimize system power consumption.
This library supports both I2C and SPI commmunication with the ICM42670S.
The device is bundled with Motion Processing Software Library. Please contact your local TDK InvenSense FAE for further information.

# Software setup
Use Arduino Library manager to find and install the ICM42670S library.

# Hardware setup

## Arduino Zero connection with  ICM42670S Daughter board

There is currently no Arduino shield for the ICM42670S.
The wiring must be done manually between the Arduino motherboard and the ICM42670S eval board.
The below wiring description is given for an Arduino Zero board, it depends on the interface to be used:

* I2C

|Arduino Zero|ICM42670S daughter board|
| --- | --- |
| 3V3        | CN4.9 & CN5.10 |
| GND        | CN5.9          |
| SDA        | CN5.4          |
...
## Arduino Zero connection with  ICM42670S Eval board

* I2C

|Arduino Zero|ICM42670S eval board|
| --- | --- |
| 5V         | CN1.19         |
| GND        | CN1.11         |
| SDA        | CN1.18         |
...
## Create ICM42670S instance

**ICM42670S(TwoWire &i2c,bool lsb)**

Create an instance of the ICM42670S that will be accessed using the specified I2C. The LSB of the I2C address can be set to 0 or 1.  
I2C default clock is 400kHz.

```C++
ICM42670S IMU(Wire,0);
```
...
```C++
ICM42670S IMU(Wire,0,1000000);
```

**ICM42670S(SPIClass &spi,uint8_t cs_id)**

Create an instance of the ICM42670S that will be accessed using the specified SPI. The IO number to be used as chip select must be specified.  
...
```C++
ICM42670S IMU(SPI,8,12000000);
```

## Initialize the ICM42670S
Call the begin method to execute the ICM42670S initialization routine. 

**int begin()**

Initializes all the required parameters in order to communicate and use the ICM42670S.
...
This method reads the ICM42670S sensor data from registers and fill the provided event structure with sensor raw data.
...
This method initializes the fifo and the interrupt of the ICM42670S. The interrupt is triggered each time there is enough samples in the fifo (as specified by fifo_watermark), and the provided handler is called.
...
This method reads the ICM42670S sensor data samples stored in the FIFO and call the provided event handler with the sample event as parameter.
...
This structure is used by the ICM42670S driver to return raw sensor data. Available data is:
...
**int startTiltDetection(uint8_t intpin, ICM42670S_irq_handler handler)**
...
**int startPedometer(uint8_t intpin, ICM42670S_irq_handler handler)**
...
**int startWakeOnMotion(uint8_t intpin, ICM42670S_irq_handler handler)**
...
**int enableInterrupt(uint8_t intpin, ICM42670S_irq_handler handler)**
...
This sketch initializes the ICM42670S with the I2C interface, and starts logging raw sensor data from IMU registers. Sensor data can be monitored on Serial monitor or Serial plotter

**Polling_SPI**

This sketch initializes the ICM42670S with the SPI interface, and starts logging raw sensor data from IMU registers. Sensor data can be monitored on Serial monitor or Serial plotter

**FIFO_Interrupt**

This sketch initializes the ICM42670S with the I2C interface and interrupt PIN2, and starts logging raw sensor data from IMU FIFO. Sensor data can be monitored on Serial monitor or Serial plotter

**APEX_Tilt**

This sketch initializes the ICM42670S with the I2C interface and interrupt PIN2, and starts the APEX Tilt detection. A TILT message is displayed on the Serial monitor when the sensor is tilted for 5 seconds.

**APEX_Pedometer**

This sketch initializes the ICM42670S with the I2C interface and interrupt PIN2, and starts the APEX Pedometer. A Pedometer status is displayed on the Serial monitor (for each step after the 5th step).

**APEX_PedometerAndTilt**

This sketch initializes the ICM42670S with the I2C interface and interrupt PIN2, and starts the APEX Pedometer and Tilt. Pedometer and Tilt status are displayed on the Serial monitor.

**APEX_WakeOnMotion**

This sketch initializes the ICM42670S with the I2C interface and interrupt PIN2, and starts the APEX Wake on Motion. A Wake-up message is displayed on the Serial monitor when the sensor detects movement.

# IMU data monitoring

When the ICM42670S IMU is logging, the Accelerometer, Gyroscope and Temperature raw data can be monitored with the Arduino Serial Plotter (Tools->Serial Plotter).
...
This sketch initializes the ICM42670S with the I2C interface and interrupt PIN2, initializes also microROS Arduino environment and starts logging raw sensor data from IMU FIFO. Sensor data are published in IMU structure. For more information, refer to MicroROS_README.md.
